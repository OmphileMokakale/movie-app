...
describe('Movie App API', function () {

	before(async function () {
		this.timeout(5000);
		await db.none(`delete from Movie App`);
		const commandText = fs.readFileSync('server/sql/movie_app.sql', 'utf-8');
	...
	it('should be able to search for movies', async () => {
		const response = await supertest(app)
			.get(`https://api.themoviedb.org/3/search/movie?api_key=1bf9f405d83b93a51712dfe88ebd97f1&query=${searchQuery}`)
			.expect(200);

            const { searchQuery } = req.params;
		assert.equal([searchResults], movieApiResponse.data.results);
	});

	it('should be able to add movies to their favorites list.', async () => {
		// add some code below
		...
		assert.equal([3, movieId, movieName, moviePosterUrl]);
	});

	it('should be able to view all movies in their own favorites list', async () => {
